	...
	FBPhotoBox.prototype = {
		init: function() {
			...
			$('.' + this.settings.containerClassName).on("click", "." + this.settings.imageClassName, function() {
				...
				// Instagram
				var url = $(this).attr('fbphotobox-src');
				var profile_id = $('#profile_id').val();
				$.get(base+'/ajax/getIGPhotoComments.php?url='+url+'&profile_id='+profile_id, function(data) {
					$('.fbphotobox-image-content-comments').html(data+'<img src="'+profile_picture+'" style="position:absolute;bottom:5px;left:10px;height:35px;width:35px;"><input type="text" class="form-control comment-input" placeholder="Enter your comment" style="position:absolute;bottom:5px;width:305px;right:5px;">');
				});
				$('#photo_url').val(url);
				$('#photo_id').val(' ');
			} else {
				...
				var profile_id = $('#profile_id').val();
				$.get(base+'/ajax/getPhotoComments.php?id='+id+'&profile_id='+profile_id, function(data) {
					$('.fbphotobox-image-content-comments').html(data+'<img src="'+profile_picture+'" style="position:absolute;bottom:5px;left:10px;height:35px;width:35px;"><input type="text" class="form-control comment-input" placeholder="Enter your comment" style="position:absolute;bottom:5px;width:305px;right:5px;">');
				});
				$('#photo_url').val(' ');
				$('#photo_id').val(id);
			...
			this.leftArrow.click(function() {
				...
				if(photo_id == "" || photo_id == " ") {
				// Instagram
				var url = $(image).attr('fbphotobox-src');
				var profile_id = $('#profile_id').val();
				$.get(base+'/ajax/getIGPhotoComments.php?url='+url+'&profile_id='+profile_id, function(data) {
					$('.fbphotobox-image-content-comments').html(data+'<img src="'+profile_picture+'" style="position:absolute;bottom:5px;left:10px;height:35px;width:35px;"><input type="text" class="form-control comment-input" placeholder="Enter your comment" style="position:absolute;bottom:5px;width:305px;right:5px;">');
				});
		...
		initDOM: function() {
			...
			'<span style="font-weight:bold;">'+gallery_desc+'</span>',
			'</div>',
			'</div>',
			...
			'<div class="fbphotobox-image-content"><div class="list-group fbphotobox-image-content-comments" style="padding-left:10px;"></div></div>',
			'</div>',
			...
			'<a class="fbphotobox-fc-close-btn" href="" style="font-size:25px;padding:5px;">&times</a>',
			'<div style="clear:both"></div>',
			'</div>',
